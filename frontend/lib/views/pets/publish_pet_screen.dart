...
class _PublishPetScreenState extends State<PublishPetScreen> {
  ...
  final TextEditingController _breedController = TextEditingController(text: 'Mestizo');
              ...
              children: [
                ...
                Expanded(
                  child: ListView(
                    ...
                    children: [
                      if (_currentStep == 0) _buildStepBasicInfo(authController),
                      if (_currentStep == 1) _buildStepHealth(),
                      if (_currentStep == 2) _buildStepBehavior(),
                      if (_currentStep == 3) _buildStepRequirements(authController),
                    ],
                  ),
                ),
                      ...
                      Expanded(
                        child: ElevatedButton(
                          ...
                          child: _isSubmitting
                              ...
                              : Text(_currentStep == 3 ? 'Publicar Mascota' : 'Siguiente'),
                        ),
                      ),
                    ],
                  ),
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              Icon(
                auth.userRole == 'shelter' ? Icons.pets : Icons.home_work_outlined,
        ...
        DropdownButtonFormField<String>(
          ...
          isExpanded: true,
          items: const [
            DropdownMenuItem(value: 'small', child: Text('Tamaño Chico (< 10 kg)', overflow: TextOverflow.ellipsis)),
            DropdownMenuItem(value: 'medium', child: Text('Tamaño Mediano (10 - 25 kg)', overflow: TextOverflow.ellipsis)),
            DropdownMenuItem(value: 'large', child: Text('Tamaño Grande (25 - 40 kg)', overflow: TextOverflow.ellipsis)),
            DropdownMenuItem(value: 'giant', child: Text('Tamaño Gigante (> 40 kg)', overflow: TextOverflow.ellipsis)),
          ],
        ...
        DropdownButtonFormField<String>(
          ...
          isExpanded: true,
          items: const [
            DropdownMenuItem(value: 'home_litter', child: Text('Nacimiento en casa (Camada propia)', overflow: TextOverflow.ellipsis)),
            DropdownMenuItem(value: 'street_rescue', child: Text('Rescatado de la calle', overflow: TextOverflow.ellipsis)),
            DropdownMenuItem(value: 'shelter_born', child: Text('Nacido en el refugio', overflow: TextOverflow.ellipsis)),
            DropdownMenuItem(value: 'relinquished', child: Text('Cesión por mudanza o fuerza mayor', overflow: TextOverflow.ellipsis)),
          ],
          onChanged: (val) {
            setState(() {
              _origin = val!;
            ...
            child: Column(
              ...
              children: [
                ...
                SwitchListTile(
                  ...
                  title: const Text('¿Completó al menos 60 días de lactancia/destete?', style: TextStyle(fontSize: 13)),
  ...
  Widget _buildStepHealth() {
    final vaccinesList = _species == 'dog'
        ? ['Antirrábica', 'Séxtuple / Óctuple', 'Pupivac (Cachorros)', 'Tos de las perreras', 'Giardia']
        : ['Antirrábica', 'Triple Felina', 'Leucemia Felina (FeLV)'];

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Estado Reproductivo y Castración:', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        DropdownButtonFormField<String>(
          ...
          items: [
            DropdownMenuItem(
              ...
              child: Text(
                _gender == 'female' ? 'Esterilizada (Cirugía realizada)' : 'Castrado (Cirugía realizada)',
  ...
  Widget _buildStepBehavior() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        Text('Nivel de energía: $_energyLevel / 5', style: const TextStyle(fontWeight: FontWeight.bold)),
        Slider(
        ...
        const SizedBox(height: 12),
        Text('Miedo o ansiedad ante tormentas / pirotecnia: $_stormAnxiety / 5',
            style: const TextStyle(fontWeight: FontWeight.bold)),
        Slider(
  ...
  Widget _buildStepRequirements(AuthController auth) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        const Text('Fotografía de la Mascota:', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        ...
        const SizedBox(height: 20),
        const Text('Requisitos para el Adoptante:', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        SwitchListTile(
        ...
        DropdownButtonFormField<String>(
          ...
          isExpanded: true,
          items: const [
            DropdownMenuItem(value: 'to_be_agreed', child: Text('Punto de encuentro a coordinar', overflow: TextOverflow.ellipsis)),
            DropdownMenuItem(value: 'home_delivery', child: Text('Se entrega a domicilio del adoptante', overflow: TextOverflow.ellipsis)),
            DropdownMenuItem(value: 'pickup_at_shelter', child: Text('Se retira en el refugio / hogar de tránsito', overflow: TextOverflow.ellipsis)),
          ],
  ...
  void _handleNextOrSubmit() async {
    if (_currentStep == 0) {
      if (_nameController.text.trim().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Por favor ingresa el nombre de la mascota')),
        );
        return;
      }
      if (_isLitter && !_weaningCompleted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('La camada debe cumplir al menos 60 días antes de ser entregada.'),
          ),
        );
        return;
      }
    ...
    if (_storyController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Por favor ingresa la historia de la mascota')),
      );
      return;
    }
    ...
    final newPet = PetModel(
      ...
      shelterId: auth.authToken ?? 'user_local',
      publisherType: auth.userRole == 'individual_rescuer' ? 'individual_rescuer' : 'shelter',
      ...
      origin: _origin,
      isLitter: _isLitter,
      weaningCompleted: _weaningCompleted,
      isVaccinated: _isVaccinated,
      vaccinesApplied: _selectedVaccines.toList(),
      dewormedInternal: _dewormedInternal,
      dewormedExternal: _dewormedExternal,
      hasMicrochip: _hasMicrochip,
      isNeutered: _reproductiveStatus == 'spayed' || _reproductiveStatus == 'neutered',
      ...
      stormAnxiety: _stormAnxiety,
      specialNeeds: _specialNeedsController.text.trim().isEmpty ? null : _specialNeedsController.text.trim(),
      story: _storyController.text.trim(),
      requiresAdoptionContract: _requiresContract,
      requiresHomeCheck: _requiresHomeCheck,
      requiresFollowupPhotos: _requiresFollowupPhotos,
    ...
    // Agregar la mascota publicada al deck
    swipeController.addPet(newPet);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('¡${newPet.name} ha sido publicado/a exitosamente en PetMatch!'),
